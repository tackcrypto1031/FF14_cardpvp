...
import { Sun, Moon, Lightbulb, RotateCcw, AlertCircle, Swords } from 'lucide-react';
...
const GameControls: React.FC<GameControlsProps> = ({
  ...
  return (
    <header className={`px-8 py-4 border-b flex items-center justify-between transition-colors duration-500 backdrop-blur-md z-50 ${isDark
      ? 'border-amber-900/20 bg-[#0d0d1a]/60 text-amber-50'
      : 'border-slate-200/60 bg-white/60 text-slate-800'
      }`}>
      {/* Brand / Title */}
      <div className="flex items-center gap-4 select-none">
        <h1 className={`text-2xl font-serif tracking-widest uppercase font-bold bg-clip-text text-transparent bg-gradient-to-r ${isDark
          ? 'from-amber-200 via-amber-500 to-amber-700'
          : 'from-slate-700 via-slate-900 to-slate-700'
          }`}>
          塔克小工具
          <span className={`block md:inline md:ml-3 text-xs md:text-sm font-normal tracking-wide opacity-60 font-sans italic ${isDark ? 'text-amber-400' : 'text-slate-500'
            }`}>
            九宮幻卡模擬器
          </span>
        </h1>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className={`p-2.5 rounded-full border transition-all duration-300 hover:scale-105 active:scale-95 shadow-sm hover:shadow-md ${isDark
        ...
        {/* Divider */}
        <div className={`w-px h-8 mx-1 transition-colors ${isDark ? 'bg-gradient-to-b from-transparent via-amber-800/40 to-transparent' : 'bg-gradient-to-b from-transparent via-slate-300 to-transparent'
          }`}></div>

        {/* Action Buttons */}
        <button
          onClick={onCalculateBestMove}
          className={`group flex items-center gap-2 px-5 py-2 rounded-md border text-xs font-bold uppercase tracking-wider transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] shadow-sm hover:shadow-md ${isDark
        ...
        <button
          onClick={onPlayAgain}
          className={`group flex items-center gap-2 px-5 py-2 rounded-md border text-xs font-bold uppercase tracking-wider transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] shadow-sm hover:shadow-md ${isDark
            ? 'border-green-600/30 bg-green-900/10 text-green-400 hover:bg-green-900/20 hover:border-green-500/50 hover:text-green-300 shadow-green-900/10'
            : 'border-green-300 bg-green-50 text-green-700 hover:bg-green-100 hover:border-green-400 shadow-green-200'
            }`}
        >
          <Swords size={14} className="transition-transform duration-300 group-hover:scale-110" />
          <span>再來一局</span>
        </button>
